...
alter table houses add column address_id int references addresses(id);
...
alter table agencies add column address_id int references addresses(id);
create table owners(
                       id serial primary key ,
                       first_name varchar,
                       last_name varchar,
...
alter table houses add column owner_id int references owners(id);
...
alter table rent_info add column owner_id int references owners(id);
alter table rent_info add column customer_id int references customers(id);
alter table rent_info add column agency_id int references agencies(id);
alter table rent_info add column house_id int references houses(id);
...
alter table agencies add column phone_number varchar;
...
alter table rent_info drop column customer_id;
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
select true from houses where price between 1500 and 2000;
-- - 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select * from addresses a inner join public.houses h on a.id = h.address_id where a.id in (5, 6, 7, 8, 9, 10);
...
select h,o.first_name, c.first_name, a.name from houses h
    ...
    inner join rent_info re on h.id = re.house_id
    inner join customers c on re.customer_id = c.id
    inner join agencies a on re.agency_id = a.id;
...
select * from houses h inner join addresses a on h.address_id = a.id
...
select o.first_name, h from houses h inner join owners o on h.owner_id = o.id
order by h.price limit 1;
-- - 23.Бир региондо жайгашкан баардык агентстволорду чыгарыныз
select * from agencies ag inner join addresses a on ag.address_id = a.id;
...
select avg(extract(year from now())-date_of_birth), h from owners inner join houses h on owners.id = h.owner_id;
